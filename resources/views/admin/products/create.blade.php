            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="sku" value="{{ old('sku') }}" required class="w-full px-3 py-2 border rounded focus:ring-gold focus:border-gold">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Short Description</label>
                        <textarea name="short_description" rows="3" class="w-full px-3 py-2 border rounded focus:ring-gold focus:border-gold">{{ old('short_description') }}</textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Full Description <span class="text-red-500">*</span></label>
                        <textarea name="description" rows="6" required class="w-full px-3 py-2 border rounded focus:ring-gold focus:border-gold">{{ old('description') }}</textarea>
                    </div>
                </div>
            </div>

            <div class="bg-white rounded-lg shadow-sm p-6" x-data="{ imgUrl: {{ Js::from(old('image_url', '')) }} }">
                <h3 class="font-semibold text-gray-700 mb-4 border-b pb-2">Media</h3>

                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Upload Product Image <span class="text-red-500">*</span></label>
                        <input type="file" name="image" accept="image/*" class="w-full px-3 py-2 border rounded focus:ring-gold focus:border-gold bg-white" @change="if($event.target.files.length) imgUrl = URL.createObjectURL($event.target.files[0])" required>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-lg shadow-sm p-6 sticky top-[88px]">
                ...
                <div class="space-y-5 mb-6">
                    ...
                    <div class="flex items-center space-x-3 bg-gray-50 p-3 rounded">
                        <input type="checkbox" name="is_featured" value="1" id="is_featured" class="w-4 h-4 text-gold rounded border-gray-300 focus:ring-gold" {{ old('is_featured') ? 'checked' : '' }}>
                ...
                <div class="space-y-4 mb-6">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" step="0.01" name="price" value="{{ old('price') }}" required class="w-full px-3 py-2 border rounded focus:ring-gold focus:border-gold">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Sale Price ($)</label>
                            <input type="number" step="0.01" name="sale_price" value="{{ old('sale_price') }}" class="w-full px-3 py-2 border rounded focus:ring-gold focus:border-gold">
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Units in Stock <span class="text-red-500">*</span></label>
                        <input type="number" name="stock" value="{{ old('stock', 0) }}" required min="0" class="w-full px-3 py-2 border rounded focus:ring-gold focus:border-gold">
                    </div>
                </div>
                ...
                <div class="space-y-4 mb-8">
                    <div>
                        ...
                        <input type="text" name="size_options" value="{{ old('size_options') }}" placeholder="e.g. XS,S,M,L,XL" class="w-full px-3 py-2 border rounded focus:ring-gold focus:border-gold">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Colors (comma separated)</label>
                        <input type="text" name="color_options" value="{{ old('color_options') }}" placeholder="e.g. Black,White,Gold" class="w-full px-3 py-2 border rounded focus:ring-gold focus:border-gold">
                    </div>
                </div>
